...
@section('content')
                ...
                <form action="{{ route('users.index') }}" method="GET" class="form-inline ml-auto">
                    <div class="input-group">
                        <input type="text" name="keyword" value="{{ request('keyword') }}"
                            class="form-control bg-light border-0 small" placeholder="Cari NIK atau Nama..."
                            aria-label="Search" aria-describedby="basic-addon2" oninput="this.form.submit()"
                            autocomplete="off" />
                        ...
                        <tbody>
                            @forelse ($users as $index => $data)
                                <tr>
                                    ...
                                    <td>{{ $data->tanggal_lahir ? \Carbon\Carbon::parse($data->tanggal_lahir)->format('d-m-Y') : '-' }}
                                    </td>
                                    <td>{{ $data->agama ?? '-' }}</td>
                                    ...
                                    <td>
                                        @if ($data->status == 'pending')
                                            <a href="{{ route('users.verifikasi', $data->id) }}"
    ...
    @foreach ($users as $data)
        <div class="modal fade" id="tolakModal{{ $data->id }}" tabindex="-1"
            aria-labelledby="tolakModalLabel{{ $data->id }}" aria-hidden="true">
